...
{
    public class ShadowmagePrefix : NPCPrefix
    {
        public override string Type => ProjectilesGroup.NAME;

        public override string Name => "Shadowmage";
        ...
        public override void AI(NPC npc)
        {
            ...
            if (distance < 450)
            {
                ...
                {
                    int p = Projectile.NewProjectile(npc.position.X, npc.position.Y, -(npc.position.X - target.position.X) / distance * 4, -(npc.position.Y - target.position.Y) / distance * 4, 468, (int)((npc.damage * 1.2) * ProjectilesGroup.DamageMultiplier), 0);
                    TriggerTimer = 0;
                }
            }
        }
