...
namespace EnemyMods.Projectiles.Duelist
{
    public class MartianFoil : ModProjectile
    {
        ...
        {
            if (Main.player[projectile.owner].FindBuffIndex(mod.BuffType("CounterCooldown")) >= 0)
            {
                Main.player[projectile.owner].buffTime[Main.player[projectile.owner].FindBuffIndex(mod.BuffType("CounterCooldown"))] -= 10;
            }
        }
    }
}
